...
export function TransactionRow({ transaction: t, categories, onUpdate, onDelete, onAddRule }: Props) {
  ...
  const rowCls = [
    ...
    deleting ? 'opacity-40' : '',
    saved ? 'bg-emerald-50' : isUncategorized
  ...
  return (
    <>
      <tr className={rowCls}>
        {/* Date */}
        <td className="px-4 py-2.5 text-xs tabular-nums text-slate-500 whitespace-nowrap">{t.date}</td>

        {/* Amount */}
        <td className={`px-4 py-2.5 text-sm font-semibold tabular-nums whitespace-nowrap ${t.type === 'income' ? 'text-emerald-600' : 'text-rose-500'}`}>
          {t.type === 'income' ? '+' : '−'}{t.amount.toLocaleString('id-ID')}
        </td>

        {/* Currency */}
        <td className="px-4 py-2.5 text-xs text-slate-400">{t.currency}</td>
        ...
        <td className="px-4 py-2.5">
          {editField === 'type' ? (
            <select
              ...
              className="rounded border border-brand-400 px-1.5 py-0.5 text-xs focus:outline-none"
            >
            ...
            <button
              ...
              className={`group flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium transition hover:ring-2 ${
                t.type === 'income'
                  ...
                  : 'bg-rose-100 text-rose-700 hover:ring-rose-300'
              }`}
            >
              {t.type}
        ...
        <td className="px-4 py-2.5">
          {editField === 'category' ? (
            ...
            >
              {saved
                ? <><svg className="h-3 w-3 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7"/></svg> saved</>
                : <>{t.category}
                    <svg className="h-2.5 w-2.5 opacity-0 group-hover:opacity-60" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536M9 13l6.586-6.586a2 2 0 012.828 2.828L11.828 15.828a2 2 0 01-1.414.586H8v-2.414a2 2 0 01.586-1.414z" />
                    </svg>
        ...
        {/* Source */}
        <td className="px-4 py-2.5 text-xs text-slate-400 max-w-[120px] truncate" title={t.source_file ?? ''}>{t.source_file}</td>
        ...
        <tr className="border-b border-brand-100 bg-brand-50/70">
          <td colSpan={8} className="px-4 py-2">
            <div className="flex items-center gap-2 text-xs text-brand-800">
              <svg className="h-3.5 w-3.5 shrink-0 text-brand-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
