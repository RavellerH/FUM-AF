...
function NumInput({ value, onChange }: { value: number; onChange: (v: number) => void }) {
  return (
    <input
      ...
      className="w-32 rounded border border-blue-400 px-2 py-0.5 text-sm focus:outline-none"
    />
  );
}
...
function SectorBar({ sectors }: { sectors: Record<string, number> }) {
  const colors = ['bg-blue-500', 'bg-indigo-400', 'bg-emerald-400', 'bg-amber-400', 'bg-rose-400'];
...
export function InvestmentPage() {
  ...
  useEffect(() => {
    if (portfolio && !draft) setDraft(JSON.parse(JSON.stringify(portfolio)));
  }, [portfolio]);
  ...
  const handleCancel = () => {
    setDraft(portfolio ? JSON.parse(JSON.stringify(portfolio)) : null);
    setEditing(false);
  };
    ...
    <div className="mx-auto max-w-5xl px-4 py-8">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">Last updated: {p.updated_at}</p>
        </div>
        <div className="flex gap-2">
          {editing ? (
            <>
              <button onClick={handleCancel} className="rounded-lg border border-gray-300 px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">Cancel</button>
            ...
            <button onClick={() => setEditing(true)} className="rounded-lg border border-gray-300 px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">
              Update snapshot
            </button>
          )}
        </div>
      ...
      <div className="mb-6 rounded-xl border border-gray-200 bg-white p-5">
        <p className="text-xs font-medium uppercase tracking-wide text-gray-400">Total Portfolio (IDR)</p>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-5">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="font-semibold text-gray-800">Stocks — StockBit</h2>
            <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${p.stocks_pnl_pct < 0 ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-700'}`}>
            ...
            <tbody>
              {p.stocks.map((h, i) => (
                <tr key={h.symbol} className="border-b border-gray-50">
                  <td className="py-2 font-medium text-gray-800">{h.symbol}</td>
                  <td className="py-2 text-center text-gray-500">
                    {editing ? (
                      <input type="number" defaultValue={h.lots} onBlur={e => updateStock(i, 'lots', parseInt(e.target.value) || 0)}
                        className="w-14 rounded border border-blue-400 px-1 text-center text-sm focus:outline-none" />
                    ) : h.lots}
                  </td>
            ...
            <div className="mt-3 grid grid-cols-3 gap-2 text-xs">
              {Object.entries(draft!.stocks_sectors).map(([name, pct]) => (
                <label key={name} className="flex flex-col gap-0.5">
                  ...
                  }} className="rounded border border-blue-400 px-1.5 py-0.5 text-sm focus:outline-none" />
                </label>
              ))}
            </div>
          ) : (
          ...
          {editing && (
            <div className="mt-3 grid grid-cols-2 gap-2">
              ...
              <label className="flex flex-col gap-0.5 text-xs">
                <span className="text-gray-500">PnL IDR</span>
        ...
        <div className="flex flex-col gap-4">
          ...
          {/* Crypto Investing */}
          <div className="rounded-xl border border-gray-200 bg-white p-5">
            <h2 className="mb-3 font-semibold text-gray-800">Crypto Investing</h2>
                ...
                <div key={c.platform} className="rounded-lg bg-orange-50 px-3 py-2.5">
                  ...
                  {editing ? (
                    <div className="mt-1 grid grid-cols-2 gap-2">
                      ...
                      <label className="flex flex-col gap-0.5 text-xs">
                        <span className="text-gray-500">Value IDR</span>
          ...
          {/* Savings */}
          <div className="rounded-xl border border-gray-200 bg-white p-5">
            <h2 className="mb-3 font-semibold text-gray-800">Savings</h2>
            {p.savings.map((sv, i) => (
              <div key={sv.name} className="rounded-lg bg-emerald-50 px-3 py-2.5">
                ...
                {editing ? (
                  <input type="number" defaultValue={sv.value_idr}
                    ...
                    className="mt-1 w-full rounded border border-blue-400 px-1.5 py-0.5 text-sm font-semibold focus:outline-none" />
                ) : (
