  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-slate-50">
      <div className="flex items-center gap-2 border-b border-slate-200 px-4 py-2">
        ...
        <span className="ml-auto text-xs text-slate-400">{text.split('\n').length} rows</span>
      </div>
      <pre className="max-h-64 overflow-auto p-4 text-xs text-slate-700 whitespace-pre-wrap">{preview}</pre>
    </div>
  );
}
